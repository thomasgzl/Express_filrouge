        ...
        router.get('/api/joueurs/supp', (req, res) => {
          ...
          router.get('/api/joueurs/ord', (req, res) => {
            ...
            router.post('/api/add', (req, res) => {
              ...
              connection.query('INSERT INTO barca_players SET ?', formData,  (err, result) => {
                if (err) {
                  console.log(err);
            ...
            router.put('/api/joueurs/:name', (req, res) => {
              ...
              const formData = req.body;
              connection.query('UPDATE barca_players SET ? WHERE name = ?', [formData, namePlayer],  (err, result) => {
                if (err) {
                  console.log(err);
              ...
              router.put('/api/joueurs/titular/:name', (req, res) => {
                const namePlayer = req.params.name;
                const formData = req.body;
                connection.query('UPDATE barca_players SET titular=1 WHERE name = ?', [formData, namePlayer],  (err, result) => {
                  if (err) {
                    console.log(err);
              ...
              //11.DELETE ENTITÉ
            router.delete('/api/joueurs/:id', (req, res) => {
              const idPlayer = req.params.id;
              connection.query('DELETE FROM barca_players WHERE id = ?', [idPlayer],  (err, result) => {
                if (err) {
                  console.log(err);
            ...
            router.delete('/api/joueurs/titular/:id', (req, res) => {
              const idPlayer = req.params.id;
              connection.query('DELETE FROM barca_players SET titular=0 WHERE id = ?', [idPlayer],  (err, result) => {
                if (err) {
                  console.log(err);
